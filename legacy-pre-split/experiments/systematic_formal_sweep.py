...
from commutativity_probe import Environment, effective_commutativity
...
def trace_environment(spec: TraceSpec) -> Environment:
    actions = action_names(spec.action_count)
    rank = {action: idx for idx, action in enumerate(actions)}
    symmetric = frozenset(tuple(sorted(pair, key=rank.__getitem__)) for pair in spec.commuting_pairs)

    def commutes(first: str, second: str) -> bool:
        return first == second or tuple(sorted((first, second), key=rank.__getitem__)) in symmetric

    def canonicalize(word: tuple[str, ...]) -> State:
        ordered = list(word)
        changed = True
        while changed:
            changed = False
            for idx in range(len(ordered) - 1):
                if rank[ordered[idx]] > rank[ordered[idx + 1]] and commutes(ordered[idx], ordered[idx + 1]):
                    ordered[idx], ordered[idx + 1] = ordered[idx + 1], ordered[idx]
                    changed = True
        return tuple(rank[action] for action in ordered)
    ...
    for state in states:
        word = tuple(actions[idx] for idx in state)
        for action in actions:
            if len(state) >= spec.depth:
                transition[(state, action)] = state
            else:
                transition[(state, action)] = canonicalize(word + (action,))
                valid.add((state, action))
    suffix = int(round(100 * spec.relation_density))
    name = f"trace_k{spec.action_count}_d{spec.depth}_rho{suffix:03d}_r{spec.relation_id}"
    return Environment(name, tuple(sorted(states)), actions, transition, frozenset(valid), spec.depth)
