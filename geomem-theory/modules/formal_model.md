...
`theoretical_effort.md` carries the broader proof agenda. `formal_theory.md` gives the current deterministic shortest-path statements, and `../../geomem-paper/manuscript/theory_appendix.md` gives the full proofs. The trace scaffold should instantiate history compression and path quotients, not replace a task-level definition of Markov and policy sufficiency.
