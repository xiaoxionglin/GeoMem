...
OBS_MODES = ("gaussian_sensory", "vector", "sparse_aliased", "egocentric_landmark", "hybrid")
TEMPORAL_MODES = ("temporal_full", "temporal_dropout", "temporal_cue_then_blank")
TEMPORAL_DROPOUT_P = 0.5
GAUSSIAN_SENSORY_SIGMA = 0.1
GAUSSIAN_SENSORY_DIM = 8
GAUSSIAN_SENSORY_SEED = 19
ACTION_START = "<START>"
ACTION_INPUT_MODES = ("none", "one_hot", "embedding")
...
LIN_BLOCK_CONFIGS = LIN_SEQUENCE_CONFIGS
...
class Dataset:
    ...
    feature_dim: int
    action_input: str = "one_hot"
    action_embedding_dim: int = 0
...
def updated_navigation_task_specs() -> list[TaskSpec]:
    specs: list[TaskSpec] = []
    for env in default_environments():
        if env.metadata.family == "trace_monoid" or env.name.startswith("trace_"):
            continue
        specs.append(TaskSpec(
            env.name,
            env,
            env.start_states[0] if env.start_states else env.states[0],
            env.metadata.geometry,
            behavior_family_for_env(env),
            max_examples=max_examples_for_env(env),
            n_goals=n_goals_for_env(env),
        ))
    return specs


def selected_task_specs(task_suite: str) -> list[TaskSpec]:
    if task_suite == "legacy":
        return task_specs()
    if task_suite == "updated_navigation":
        return updated_navigation_task_specs()
    raise ValueError(f"unknown task suite: {task_suite}")


def behavior_family_for_env(env: Environment) -> str:
    by_name = {
        "torus_lattice": "metric_landmark",
        "clamped_lattice": "metric_landmark",
        "bottleneck_rooms": "bottleneck_route",
        "aliased_rooms": "repeated_room",
        "aliased_t_maze": "delayed_landmark_choice",
        "landmark_gap_detour": "landmark_gap_detour",
        "shortcut_route_choice": "shortcut_route_memory",
        "aliased_loop_rooms": "repeated_room_loop",
    }
    if env.name in by_name:
        return by_name[env.name]
    if env.name.startswith("room_graph_"):
        return "room_graph_navigation"
    if env.name.startswith("shortcut_tree_"):
        return "shortcut_tree_navigation"
    return env.metadata.family if env.metadata.family != "unspecified" else "formal"


def max_examples_for_env(env: Environment) -> int:
    if env.metadata.family in {"cue_gap", "route_choice"}:
        return 180
    if env.metadata.family in {"rooms", "room_graph", "aliased_navigation"}:
        return 420
    if env.metadata.family in {"tree", "shortcut_tree"}:
        return 300
    return 220


def n_goals_for_env(env: Environment) -> int:
    if env.metadata.family in {"cue_gap", "route_choice"}:
        return 2
    return 3
...
def gaussian_sensory_feature(state: State, task: TaskSpec) -> np.ndarray:
    channel = gaussian_sensory_channel(
        task.env,
        sigma=GAUSSIAN_SENSORY_SIGMA,
        dim=GAUSSIAN_SENSORY_DIM,
        seed=GAUSSIAN_SENSORY_SEED,
    )
    obs_seed = stable_seed("gaussian_sensory", task.name, state)
    return np.asarray(channel.observe(state, obs_seed), dtype=np.float32)


def action_token(action: str | None, action_labels: list[str]) -> np.ndarray:
    values = np.zeros(len(action_labels) + 1, dtype=np.float32)
    if action is None or action == ACTION_START:
        values[0] = 1.0
    else:
        values[1 + action_labels.index(action)] = 1.0
    return values


def action_embedding(action: str | None, action_labels: list[str], dim: int) -> np.ndarray:
    label = ACTION_START if action is None or action == ACTION_START else action
    seed = stable_seed("action_embedding", tuple(action_labels), label, dim)
    rng = np.random.default_rng(seed)
    return rng.normal(0.0, 1.0 / math.sqrt(dim), size=dim).astype(np.float32)


def action_features(action: str | None, action_labels: list[str], mode: str, embedding_dim: int) -> np.ndarray:
    if mode == "none":
        return np.zeros(0, dtype=np.float32)
    if mode == "one_hot":
        return action_token(action, action_labels)
    if mode == "embedding":
        return action_embedding(action, action_labels, embedding_dim)
    raise ValueError(f"unknown action input mode: {mode}")


def path_previous_actions(
    path: list[State],
    task: TaskSpec,
    action_labels: list[str],
    graph: dict[State, list[tuple[str, State]]] | None = None,
) -> list[str | None]:
    previous: list[str | None] = [None]
    for source, target in zip(path, path[1:]):
        if graph is None:
            action = next(
                (candidate for candidate in action_labels if task.env.step(source, candidate) == target),
                None,
            )
        else:
            action = next(
                (candidate for candidate, neighbor in graph[source] if neighbor == target and candidate in action_labels),
                None,
            )
        if action is None:
            raise ValueError(f"cannot infer action from {source} to {target} in {task.name}")
...
def observation_frame(
    ...
    return np.concatenate([
        state_feature(state, task, mode, state_index),
        state_feature(goal, task, mode, state_index),
        action_features(previous_action, action_labels, action_input, action_embedding_dim),
    ])
...
def state_feature(state: State, task: TaskSpec, mode: str, state_index: dict[State, int]) -> np.ndarray:
    ...
    if mode == "gaussian_sensory":
        return gaussian_sensory_feature(state, task)
    if mode == "vector":
        if task.name.startswith("trace"):
...
def sequence_features(
    ...
    previous_actions = previous_actions or path_previous_actions(path, task, action_labels, graph)
    if len(previous_actions) != len(path):
        raise ValueError("previous_actions must align with path")
    ...
    for idx, state in enumerate(path):
        ...
        if not keep[idx]:
            state_dim = len(state_feature(state, task, mode, state_index))
            frame[:state_dim] = 0.0
        frames.append(frame)
...
def build_dataset(
    ...
    if action_input not in ACTION_INPUT_MODES:
        raise ValueError(f"unknown action input mode: {action_input}")
    if action_input == "embedding" and action_embedding_dim <= 0:
        raise ValueError("action_embedding_dim must be positive for embedding action input")
    if task.name in {"aliased_t_maze", "long_aliased_t_maze"}:
        return build_tmaze_dataset(task, obs_mode, temporal_mode, temporal_dropout_p, seed, action_input, action_embedding_dim)
    if task.name in {"landmark_gap_detour", "shortcut_route_choice"}:
        return build_choice_route_dataset(task, obs_mode, temporal_mode, temporal_dropout_p, seed, action_input, action_embedding_dim)
    rng = Random(seed)
    ...
    for idx in range(total):
        ...
        for _ in range(max_steps):
            ...
            frame = observation_frame(
                ...
                dataset.action_embedding_dim,
            )
            if dataset.temporal_mode != "temporal_full":
                state_dim = len(state_feature(state, dataset.task, dataset.obs_mode, state_index))
...
def evaluate_dataset(dataset: Dataset, dataset_seed: int, seed: int = 0) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    weight_rows.append({
        ...
        "action_input": dataset.action_input,
        "action_embedding_dim": dataset.action_embedding_dim,
        "model": "numpy_rnn",
        "sequence_r": "",
        "sequence_l": "",
        "sequence_ell": "",
    ...
    for sequence_r, sequence_l in LIN_BLOCK_CONFIGS:
        model_seed = seed + 900 + 17 * sequence_r + 31 * sequence_l
        train_states, w_rec = lin_block_sequence_states(dataset.x_train, sequence_r, sequence_l)
        test_states, _ = lin_block_sequence_states(dataset.x_test, sequence_r, sequence_l)
        block_predictor = fit_linear_predictor(train_states, y_train)
        block_preds = block_predictor(test_states)

        def predict_lin_block(
            xs: list[np.ndarray],
            *,
            fixed_r: int = sequence_r,
            fixed_l: int = sequence_l,
        ) -> np.ndarray:
            states, _ = lin_block_sequence_states(xs, fixed_r, fixed_l)
            return block_predictor(states)
        ...
        rows.append(result_row(
            ...
            dataset_seed,
            model_seed,
            accuracy_score(y_test, list(block_preds)),
            greedy_success(dataset, predict_lin_block),
            sequence_r=sequence_r,
            sequence_l=sequence_l,
...
def summarize_rows(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        grouped[(
            ...
            str(row["action_input"]),
            str(row["action_embedding_dim"]),
            str(row["model"]),
        )].append(row)
    summary: list[dict[str, object]] = []
    for (task, geometry, behavior_family, obs_mode, temporal_mode, action_input, action_embedding_dim, model), group in sorted(grouped.items()):
...
def memory_advantage_summary(summary: list[dict[str, object]]) -> list[dict[str, object]]:
    by_task_obs: dict[tuple[str, str, str, str], dict[str, dict[str, object]]] = defaultdict(dict)
    for row in summary:
        by_task_obs[(str(row["task"]), str(row["obs_mode"]), str(row["temporal_mode"]), str(row["action_input"]))][str(row["model"])] = row
    out: list[dict[str, object]] = []
    for (task, obs_mode, temporal_mode, action_input), model_rows in sorted(by_task_obs.items()):
        memoryless = max(
        ...
        recurrent_candidates = {
            ...
            if (
                model == "numpy_rnn"
                or model.startswith("reservoir_")
                or model.startswith("lin_sequence_")
                or model.startswith("lin_block_")
            )
        }
        ...
        out.append({
            ...
            "temporal_mode": temporal_mode,
            "action_input": action_input,
            "action_embedding_dim": best_row["action_embedding_dim"],
...
def heatmap(rows: list[dict[str, object]], *, obs_mode: str, temporal_mode: str, metric: str, path: Path, tasks: list[TaskSpec] | None = None) -> None:
    tasks = tasks or task_specs()
    ...
    cell_w = (width - margin_left - margin_right) / len(task_names)
...
def action_ablation_plot(summary: list[dict[str, object]], path: Path, *, obs_mode: str = "gaussian_sensory", temporal_mode: str = "temporal_full") -> None:
    models = ("memoryless_linear", "numpy_rnn", "reservoir_nilpotent", "lin_sequence_front_bands_r3_l4")
    action_inputs = ACTION_INPUT_MODES
    grouped: dict[tuple[str, str], list[float]] = defaultdict(list)
    for row in summary:
        if row["obs_mode"] == obs_mode and row["temporal_mode"] == temporal_mode and row["model"] in models:
            grouped[(str(row["action_input"]), str(row["model"]))].append(float(row["action_accuracy_mean"]))
    values = {key: float(np.mean(vals)) for key, vals in grouped.items()}
    width, height = 720, 420
    margin_left, margin_right, margin_top, margin_bottom = 210, 40, 84, 70
    cell_w = (width - margin_left - margin_right) / len(action_inputs)
    cell_h = (height - margin_top - margin_bottom) / len(models)
    ...
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<rect width="100%" height="100%" fill="#ffffff"/>',
        '<style>text{font-family:Arial,Helvetica,sans-serif;fill:#1f2933}.title{font-size:30px;font-weight:700}.label{font-size:18px}.cell{font-size:18px;font-weight:700}</style>',
        f'<text class="title" x="50" y="38">Gaussian action-input ablation</text>',
        f'<text class="label" x="50" y="62">Mean action accuracy; {esc(obs_mode)} / {esc(temporal_mode)}</text>',
    ]
    for row_idx, model in enumerate(models):
        y0 = margin_top + row_idx * cell_h
        parts.append(f'<text class="label" text-anchor="end" x="{margin_left - 12}" y="{y0 + cell_h / 2 + 5:.1f}">{esc(model.replace("_", " "))}</text>')
        for col_idx, action_input in enumerate(action_inputs):
            x0 = margin_left + col_idx * cell_w
            value = values.get((action_input, model), float("nan"))
            fill = "#f5f7fa" if value != value else color(value)
            parts.append(f'<rect x="{x0:.1f}" y="{y0:.1f}" width="{cell_w:.1f}" height="{cell_h:.1f}" fill="{fill}" stroke="#ffffff" stroke-width="2"/>')
            text = "NA" if value != value else f"{value:.2f}"
    ...
    for col_idx, action_input in enumerate(action_inputs):
        x0 = margin_left + col_idx * cell_w + cell_w / 2
        parts.append(f'<text class="label" text-anchor="middle" x="{x0:.1f}" y="{height - 32}">{esc(action_input.replace("_", " "))}</text>')
    parts.append("</svg>")
    path.write_text("\n".join(parts), encoding="utf-8")
...
def memory_advantage_plot(rows: list[dict[str, object]], path: Path) -> None:
    filtered = [
        row for row in rows
        if row["obs_mode"] in {"gaussian_sensory", "sparse_aliased", "hybrid"} and row["temporal_mode"] == "temporal_full"
    ]
    if not filtered:
        path.write_text(
            '<svg xmlns="http://www.w3.org/2000/svg" width="640" height="160" viewBox="0 0 640 160">\n'
            '<rect width="100%" height="100%" fill="#ffffff"/>\n'
            '<text x="40" y="80" font-family="Arial,Helvetica,sans-serif" font-size="20" fill="#1f2933">No memory-advantage rows available.</text>\n'
            "</svg>",
            encoding="utf-8",
        )
        return
...
def temporal_sparsity_summary(summary: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in summary:
        grouped[(str(row["obs_mode"]), str(row["temporal_mode"]), str(row["action_input"]), str(row["model"]))].append(row)
    out: list[dict[str, object]] = []
    for (obs_mode, temporal_mode, action_input, model), group in sorted(grouped.items()):
        ...
        out.append({
            ...
            "temporal_mode": temporal_mode,
            "action_input": action_input,
            "action_embedding_dim": group[0]["action_embedding_dim"],
            "model": model,
            "n_tasks": len(group),
            "action_accuracy_mean": float(np.mean(accuracies)),
            "action_accuracy_sem": float(np.std(accuracies, ddof=1) / math.sqrt(len(accuracies))) if len(accuracies) > 1 else 0.0,
...
def temporal_sparsity_advantage(advantage_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in advantage_rows:
        grouped[(str(row["obs_mode"]), str(row["temporal_mode"]), str(row["action_input"]))].append(row)
    out: list[dict[str, object]] = []
    for (obs_mode, temporal_mode, action_input), group in sorted(grouped.items()):
...
def lin_sequence_injection_summary(summary: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in lin_sequence_summary(summary):
        if int(row["sequence_r"]) == LIN_SEQUENCE_REFERENCE[0] and int(row["sequence_l"]) == LIN_SEQUENCE_REFERENCE[1]:
            grouped[(str(row["obs_mode"]), str(row["temporal_mode"]), str(row["action_input"]), str(row["input_injection"]))].append(row)
    out: list[dict[str, object]] = []
    for (obs_mode, temporal_mode, action_input, input_injection), group in sorted(grouped.items()):
        accuracies = np.asarray([float(row["action_accuracy_mean"]) for row in group], dtype=float)
...
def parse_csv_arg(value: str, allowed: tuple[str, ...], *, allow_all: bool = False) -> tuple[str, ...]:
    if allow_all and value == "all":
        return allowed
    items = tuple(item.strip() for item in value.split(",") if item.strip())
    unknown = [item for item in items if item not in allowed]
    if unknown:
        raise ValueError(f"unknown values {unknown}; expected one of {allowed}")
    if not items:
        raise ValueError("at least one value is required")
    return items


def output_path(prefix: str, suffix: str) -> Path:
    canonical = {
        "benchmark": "rnn_navigation_benchmark.csv",
        "summary": "rnn_navigation_summary.csv",
        "memory_advantage": "rnn_memory_advantage.csv",
        "temporal_sparsity_summary": "rnn_temporal_sparsity_summary.csv",
        "temporal_sparsity_advantage": "rnn_temporal_sparsity_advantage.csv",
        "lin_sequence_summary": "rnn_lin_sequence_summary.csv",
        "lin_sequence_injection": "rnn_lin_sequence_injection.csv",
        "lin_long_sequence_benchmark": "rnn_lin_long_sequence_benchmark.csv",
        "lin_long_sequence_summary": "rnn_lin_long_sequence_summary.csv",
        "lin_long_architecture_families": "rnn_lin_long_architecture_families.csv",
        "weight_analysis": "rnn_weight_analysis.csv",
        "memory_advantage_svg": "rnn_memory_advantage.svg",
        "temporal_sparsity_reservoirs_svg": "rnn_temporal_sparsity_reservoirs.svg",
        "lin_sequence_grid_svg": "rnn_lin_sequence_grid.svg",
        "lin_long_sequence_grid_svg": "rnn_lin_long_sequence_grid.svg",
        "lin_long_architecture_comparison_svg": "rnn_lin_long_architecture_comparison.svg",
    }
    if prefix == "rnn_navigation" and suffix in canonical:
        return FIGURES / canonical[suffix]
    extension = "svg" if suffix.endswith("_svg") else "csv"
    stem = suffix.removesuffix("_svg")
    return FIGURES / f"{prefix}_{stem}.{extension}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--obs-modes", default=",".join(OBS_MODES), help="Comma-separated observation modes.")
    parser.add_argument("--task-suite", default="updated_navigation", choices=("legacy", "updated_navigation"))
    parser.add_argument("--action-input", default="one_hot", help="none, one_hot, embedding, all, or comma-separated modes.")
    parser.add_argument("--action-embedding-dim", type=int, default=ACTION_EMBEDDING_DIM)
    parser.add_argument("--output-prefix", default="rnn_navigation")
    parser.add_argument("--temporal-modes", default=",".join(TEMPORAL_MODES), help="Comma-separated temporal modes.")
    parser.add_argument("--skip-long-architecture", action="store_true")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    obs_modes = parse_csv_arg(args.obs_modes, OBS_MODES)
    ...
    tasks = selected_task_specs(args.task_suite)
    ...
    write_csv(output_path(args.output_prefix, "benchmark"), rows)
    write_csv(output_path(args.output_prefix, "summary"), summary_rows)
    write_csv(output_path(args.output_prefix, "memory_advantage"), advantage_rows)
    write_csv(output_path(args.output_prefix, "temporal_sparsity_summary"), temporal_summary_rows)
    write_csv(output_path(args.output_prefix, "temporal_sparsity_advantage"), temporal_advantage_rows)
    write_csv(output_path(args.output_prefix, "lin_sequence_summary"), lin_rows)
    write_csv(output_path(args.output_prefix, "lin_sequence_injection"), lin_injection_rows)
    write_csv(output_path(args.output_prefix, "weight_analysis"), weight_rows)
    for obs_mode in obs_modes:
        heatmap(rows, obs_mode=obs_mode, temporal_mode="temporal_full", metric="action_accuracy", path=FIGURES / f"{args.output_prefix}_{obs_mode}_accuracy.svg", tasks=tasks)
        heatmap(rows, obs_mode=obs_mode, temporal_mode="temporal_full", metric="greedy_success", path=FIGURES / f"{args.output_prefix}_{obs_mode}_success.svg", tasks=tasks)
    if "gaussian_sensory" in obs_modes and len(action_inputs) > 1:
        action_ablation_plot(summary_rows, FIGURES / f"{args.output_prefix}_action_ablation.svg")
    memory_advantage_plot(advantage_rows, output_path(args.output_prefix, "memory_advantage_svg"))
    temporal_reservoir_plot(summary_rows, output_path(args.output_prefix, "temporal_sparsity_reservoirs_svg"))
    lin_sequence_grid_plot(summary_rows, output_path(args.output_prefix, "lin_sequence_grid_svg"))
    if not args.skip_long_architecture:
        ...
        write_csv(output_path(args.output_prefix, "lin_long_sequence_benchmark"), long_lin_rows)
        write_csv(output_path(args.output_prefix, "lin_long_sequence_summary"), long_lin_summary_rows)
        write_csv(output_path(args.output_prefix, "lin_long_architecture_families"), long_architecture_family_rows)
        lin_long_sequence_plot(long_lin_summary_rows, output_path(args.output_prefix, "lin_long_sequence_grid_svg"))
        long_architecture_plot(long_lin_summary_rows, output_path(args.output_prefix, "lin_long_architecture_comparison_svg"))
    print("Generated:")
    generated = [
        output_path(args.output_prefix, "benchmark"),
        output_path(args.output_prefix, "summary"),
        output_path(args.output_prefix, "memory_advantage"),
        output_path(args.output_prefix, "temporal_sparsity_summary"),
        output_path(args.output_prefix, "temporal_sparsity_advantage"),
        output_path(args.output_prefix, "lin_sequence_summary"),
        output_path(args.output_prefix, "lin_sequence_injection"),
        output_path(args.output_prefix, "weight_analysis"),
        *(FIGURES / f"{args.output_prefix}_{obs_mode}_{metric}.svg" for obs_mode in obs_modes for metric in ("accuracy", "success")),
        output_path(args.output_prefix, "memory_advantage_svg"),
        output_path(args.output_prefix, "temporal_sparsity_reservoirs_svg"),
        output_path(args.output_prefix, "lin_sequence_grid_svg"),
    ]
    if "gaussian_sensory" in obs_modes and len(action_inputs) > 1:
        generated.append(FIGURES / f"{args.output_prefix}_action_ablation.svg")
    if not args.skip_long_architecture:
        generated.extend([
            output_path(args.output_prefix, "lin_long_sequence_benchmark"),
            output_path(args.output_prefix, "lin_long_sequence_summary"),
            output_path(args.output_prefix, "lin_long_architecture_families"),
            output_path(args.output_prefix, "lin_long_sequence_grid_svg"),
            output_path(args.output_prefix, "lin_long_architecture_comparison_svg"),
        ])
